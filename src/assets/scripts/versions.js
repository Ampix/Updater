...
async function loadup(type) {
  if (type == "builders") {
    ...
    fs.readFile(dirs.builders + "\\ampixupdater\\type.txt", (err, data) => {
      ...
      updatetext(
        "statuswarn",
        "builders",
        "Itt másik modpack van telepítve először töröld le azt a gombbal."
      );
    });
    request.get(
      "https://cdn.ampix.hu/builders/ver.txt",
      function (err, res, body) {
        if (body == undefined)
          return updatetext(
            "newver",
            "builders",
            "Hiba: Nem lehet elérni a rendszert"
          );
  ...
  if (type == "twigmod2") {
    ...
    request.get(
      "https://cdn.ampix.hu/twigmod2/ver.txt",
      function (err, res, body) {
        if (body == undefined)
          return updatetext(
            "newver",
